//// kir.toml 파싱/직렬화, gleam.toml 및 package.json 마이그레이션 지원
...
// ---------------------------------------------------------------------------
// gleam.toml 읽기 (kir init 용)
// ---------------------------------------------------------------------------

/// gleam.toml을 읽어 KirConfig로 변환 (kir init 마이그레이션)
pub fn read_gleam_toml(directory: String) -> Result(KirConfig, ConfigError) {
...
fn decode_gleam_toml(doc: Dict(String, Toml)) -> Result(KirConfig, ConfigError) {
  use name <- result.try(
...
// ---------------------------------------------------------------------------
// package.json 읽기 (kir init 용)
// ---------------------------------------------------------------------------

/// package.json에서 npm 의존성 목록 추출
pub fn read_package_json(
  directory: String,
) -> Result(List(Dependency), ConfigError) {
